...
getPlayerName = () => {
  playerName = readline.question(`What's your name, being? `, {
  ...
  if (
    ...
    console.log(
      `Really? You don't have a name? Okay then, your name is ${defaultName}.`
    );
    playerName = defaultName;
  } else {
    console.log(
      `${playerName}! Outstanding! A name becoming of an intergalactic dictator-to-be.`
    );
  }
};
...
getPlayerPlanet = () => {
  if (
    readline.question(
      `And from whence do you hail, ${playerName}?`.toLowerCase()
    ) == "earth"
  ) {
